...
class SitewebAdmin(admin.ModelAdmin):
	list_display = ('view_photoProfil', 'view_photoCouverture', 'nom', 'prenom', 'email', 'telephone', 'adresse', 'date_add', 'date_update', 'status')
	date_hierarchy = 'date_add'
	list_per_page = 10
	list_editable = ['email', 'status']

	def view_photoProfil(self, obj):
		return mark_safe(f'<img src="{obj.photoProfil.url}" style="height:100px; width:120px">')
	view_photoProfil.short_description = 'Apercu de view_photoProfil'

	def view_photoCouverture(self, obj):
		return mark_safe(f'<img src="{obj.photoCouverture.url}" style="height:100px; width:120px">')
